...
async def listado_cit_clientes_recuperaciones(
    ...
    if "CIT CLIENTES RECUPERACIONES" not in current_user.permissions or current_user.permissions["CIT CLIENTES RECUPERACIONES"] < Permiso.VER:
...
async def detalle_cit_cliente_recuperacion(
    ...
    if "CIT CLIENTES RECUPERACIONES" not in current_user.permissions or current_user.permissions["CIT CLIENTES RECUPERACIONES"] < Permiso.VER:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Forbidden")
    try:
        cit_cliente_recuperacion = get_cit_cliente_recuperacion(db, cit_cliente_recuperacion_id=cit_cliente_recuperacion_id)
    except (IsDeletedException, NotExistsException) as error:
        raise HTTPException(status_code=status.HTTP_406_NOT_ACCEPTABLE, detail=f"Not acceptable: {str(error)}") from error
